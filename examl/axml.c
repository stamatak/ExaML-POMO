...
boolean isPomo(int dataType)
{
  if(dataType == POMO_16 || dataType == POMO_64)
    return TRUE;
  else
    return FALSE;
}
...
static boolean setupTree (tree *tr)
{
  ...
  tr->partitionWeights       = (double *)malloc(sizeof(double) * (size_t)tr->NumberOfModels);
  ...
  if (!(tr->nodep = (nodeptr *) malloc(2 * (size_t)tr->mxtips * sizeof(nodeptr))))
    {
...
{  
  ...
  if(strcmp(model, "PLAIN\0") == 0)
    {
      tr->rateHetModel = PLAIN;
      return 1;
    }
  ...
  if(processID == 0)
    {
      ...
      printf("              select \"-m PLAIN\" for a model without rate heterogeneity, works only for POMO data!\n");
      printf("\n");
...
static void printModelAndProgramInfo(tree *tr, analdef *adef, int argc, char *argv[])
{
  if(processID == 0)
	...
	case CAT:
	  ...
	  break;
	case PLAIN:
	  printBoth(infoFile, "Not using any model of among site rate heterogeneity\n\n");
	  break;
	default:
	  assert(0);
	}               
      ...
      for(model = 0; model < tr->NumberOfModels; model++)
	{
	  ...
	  switch(tr->partitionData[model].dataType)
	    ...
	    case BINARY_DATA:
	      ...
	      break;
	      //mth just some print out
	    case POMO_16:
	      printBoth(infoFile, "DataType: POMO_16\n");	      
	      printBoth(infoFile, "Substitution Matrix: some weird stuff\n");
	      break;
	    case POMO_64:
	      printBoth(infoFile, "DataType: POMO_64\n");	      
	      printBoth(infoFile, "Substitution Matrix: some weird stuff\n");
	      break;
...
static void printModelParams(tree *tr, analdef *adef, int treeIteration)
{
  ...
  for(model = 0; model < tr->NumberOfModels; model++)
	...
	{
	  printBothOpenDifferentFile(fileName, "\nPomo K:   %f\n", tr->partitionData[model].pomoK);
	  printBothOpenDifferentFile(fileName, "Pomo Phi: %f\n", tr->partitionData[model].pomoPhi);
	  printBothOpenDifferentFile(fileName, "Pomo N:   %d\n\n", tr->partitionData[model].pomoN);
	}
	...
	case POMO_16:
	  {	    
	    ...
	    printRatesDNA_BIN(16, r, freqNames, fileName);
	    printBothOpenDifferentFile(fileName, "\n");
	    printFreqs(16, f, freqNames, fileName);
	  }
	  break;
	case POMO_64:
	  {	    	    
	    char *freqNames[64] = {"A", "C", "G", "T", 
				   "A10|C1", "A9|C2", "A8|C3", "A7|C4", "A6|C5", "A5|C6", "A4|C7", "A3|C8", "A2|C9", "A1|C10",  
				   "A10|G1", "A9|G2", "A8|G3", "A7|G4", "A6|G5", "A5|G6", "A4|G7", "A3|G8", "A2|G9", "A1|G10",
				   "A10|T1", "A9|T2", "A8|T3", "A7|T4", "A6|T5", "A5|T6", "A4|T7", "A3|T8", "A2|T9", "A1|T10", 
				   "C10|G1", "C9|G2", "C8|G3", "C7|G4", "C6|G5", "C5|G6", "C4|G7", "C3|G8", "C2|G9", "C1|G10",
				   "C10|T1", "C9|T2", "C8|T3", "C7|T4", "C6|T5", "C5|T6", "C4|T7", "C3|T8", "C2|T9", "C1|T10",
				   "G10|T1", "G9|T2", "G8|T3", "G7|T4", "G6|T5", "G5|T6", "G4|T7", "G3|T8", "G2|T9", "G1|T10"};


	    printRatesDNA_BIN(64, r, freqNames, fileName);
	    printBothOpenDifferentFile(fileName, "\n");
	    printFreqs(64, f, freqNames, fileName);
	  }
	  break;
	default:
	  assert(0);
...
static void error_MPI_Exit(void)  __attribute__ ((noreturn));
...
{ 
  ...
  for(model = 0; model < tr->NumberOfModels; model++)
	...
	*pl = getPartitionLengths(&(tr->partitionData[model])); 
      ...
      for (t = 0; t < tr->maxThreadsPerModel; ++t)
	{
      ...
      tr->partitionData[model].left              = (double *)malloc_aligned((size_t)pl->leftLength * ((size_t)maxCategories + 1) * sizeof(double));
      tr->partitionData[model].right             = (double *)malloc_aligned((size_t)pl->rightLength * ((size_t)maxCategories + 1) * sizeof(double));
      tr->partitionData[model].EIGN              = (double*)malloc((size_t)pl->eignLength * sizeof(double));
      tr->partitionData[model].EV                = (double*)malloc_aligned((size_t)pl->evLength * sizeof(double));
      tr->partitionData[model].EI                = (double*)malloc((size_t)pl->eiLength * sizeof(double));
      
      tr->partitionData[model].substRates        = (double *)malloc((size_t)pl->substRatesLength * sizeof(double));
      ...
      tr->partitionData[model].freqExponents     = (double*)malloc((size_t)pl->frequenciesLength * sizeof(double));
      tr->partitionData[model].empiricalFrequencies       = (double*)malloc((size_t)pl->frequenciesLength * sizeof(double));
      tr->partitionData[model].tipVector         = (double *)malloc_aligned((size_t)pl->tipVectorLength * sizeof(double));
	...
	{	  	  
	  ...
	  for(k = 0; k < 4; k++)
	    {	    
	      ...
	      tr->partitionData[model].substRates_LG4[k]        = (double *)malloc((size_t)pl->substRatesLength * sizeof(double));
	      tr->partitionData[model].frequencies_LG4[k]       = (double*)malloc((size_t)pl->frequenciesLength * sizeof(double));
	      tr->partitionData[model].tipVector_LG4[k]         = (double *)malloc_aligned((size_t)pl->tipVectorLength * sizeof(double));
	    }
	}


      tr->partitionData[model].symmetryVector    = (int *)malloc((size_t)pl->symmetryVectorLength  * sizeof(int));
      tr->partitionData[model].frequencyGrouping = (int *)malloc((size_t)pl->frequencyGroupingLength  * sizeof(int));
      
      tr->partitionData[model].perSiteRates      = (double *)malloc(sizeof(double) * (size_t)tr->maxCategories);
      ...
      tr->partitionData[model].xVector = (double **)malloc(sizeof(double*) * (size_t)tr->mxtips);   
      	
      for(j = 0; j < (size_t)tr->mxtips; j++)	        	  	  	  	 
	  tr->partitionData[model].xVector[j]   = (double*)NULL;   

      tr->partitionData[model].xSpaceVector = (size_t *)calloc((size_t)tr->mxtips, sizeof(size_t));  
      ...
      if(width > 0 && tr->saveMemory)
	{
	  ...
	  len = (size_t)tr->partitionData[model].gapVectorLength * 2 * (size_t)tr->mxtips; 
  ...
  {      
    ...
    */
    ...
    unsigned long 
      *modelWeights = (unsigned long*) calloc((size_t)tr->NumberOfModels, sizeof(unsigned long)); 
	...
	for(k = 0; k < partition.width; ++k)
	  modelWeights[model] += (unsigned long) partition.wgt[k]; 
      }
	  ...
	  if(width > 0)
		...
		for(i = 0; i < width; i++)
		  if(tr->partitionData[model].yVector[j][i] == undetermined)
		    tr->partitionData[model].gapVector[(size_t)tr->partitionData[model].gapVectorLength * j + i / 32] |= mask32[i % 32];	    
	    }     
	}
    }
}
...
Please set #define NUM_BRANCHES in axml.h to %d (or higher) and recompile %s\n", 
  ...
  tr->executeModel   = (boolean *)calloc((size_t)tr->NumberOfModels, sizeof(boolean));
  ...
  if(tr->searchConvergenceCriterion && processID == 0)
    {                     
      ...
      tr->h = initHashTable((unsigned int)tr->mxtips * 4);     
    }
...
static void optimizeTrees(tree *tr, analdef *adef)
{
  ...
  treeOffsets = (exa_off_t *)malloc(sizeof(exa_off_t) * ((size_t)tr->numberOfTrees + 1));

  tr->likelihoods = (double *)malloc(sizeof(double) * (size_t)tr->numberOfTrees);
  ...
  if(adef->useCheckpoint)
	...
	modOpt(tr, 0.1, adef, i);
      ...
      memcpy(&(tr->treeStrings[(size_t)tr->treeStringLength * (size_t)i]), tr->tree_string, sizeof(char) * (size_t)tr->treeStringLength);
  ...
  /* generate the ExaML output file names and store them in strings */
      ...
      if(tr->rateHetModel == CAT)
	{
	  ...
	  error_MPI_Exit(); 
       ...
       if(tr->rateHetModel == PLAIN)
	{
	  printBothOpen("Error: There is no MIC support yet for the PLAIN model!\n\n");	  
	  error_MPI_Exit(); 
	}      
#endif
      ...
      for(model = 0; model < tr->NumberOfModels; model++)
	{
	  ...
	  if(tr->partitionData[model].states == 2)
	    countBinary++;	  	  
	  if(tr->partitionData[model].states == 16)
	    countPomo16++;
	  if(tr->partitionData[model].states == 64)
	    countPomo64++;	  
	}

      if(countPomo16 > 0 && countPomo16 != tr->NumberOfModels)
	{
	  printBothOpen("Error: not all partitions are POMO16 type partitions\n\n");	  
	  error_MPI_Exit();
	}
      
       if(countPomo64 > 0 && countPomo64 != tr->NumberOfModels)
	{
	  printBothOpen("Error: not all partitions are POMO64 type partitions\n\n");	  
	  error_MPI_Exit();
	}
       
       if(tr->rateHetModel == PLAIN && !(countPomo64 == tr->NumberOfModels || countPomo16 == tr->NumberOfModels))
	 {	  
	   printBothOpen("Error: you can only use the plain model of rate heterogeneity for POMO-type partitions\n\n");	  
	   error_MPI_Exit();
	 }
