...
static void writeTree(tree *tr, FILE *f)
{
  ...
  myBinFwrite(tr->nodeBaseAddress, sizeof(node), (size_t)x, f);
...
static void writeCheckpointInner(tree *tr, int *rateCategory, double *patrat, analdef *adef)
{
  ...
  if(tr->constraintTree)
    myBinFwrite(tr->constraintVector, sizeof(int), 2 * (size_t)tr->mxtips, f);  

  myBinFwrite(tr->tree0, sizeof(char), (size_t)tr->treeStringLength, f);
  myBinFwrite(tr->tree1, sizeof(char), (size_t)tr->treeStringLength, f);
  ...
  for(model = 0; model < tr->NumberOfModels; model++)
	...
	dataType = tr->partitionData[model].dataType;
      ...
      myBinFwrite(tr->partitionData[model].perSiteRates, sizeof(double), (size_t)tr->maxCategories, f);
      myBinFwrite(tr->partitionData[model].EIGN, sizeof(double), (size_t)pLengths[dataType].eignLength, f);
      myBinFwrite(tr->partitionData[model].EV, sizeof(double),  (size_t)pLengths[dataType].evLength, f);
      myBinFwrite(tr->partitionData[model].EI, sizeof(double),  (size_t)pLengths[dataType].eiLength, f);  

      myBinFwrite(tr->partitionData[model].freqExponents, sizeof(double),  (size_t)pLengths[dataType].frequenciesLength, f);
      myBinFwrite(tr->partitionData[model].frequencies,   sizeof(double), (size_t)pLengths[dataType].frequenciesLength, f);
      myBinFwrite(tr->partitionData[model].tipVector,     sizeof(double),  (size_t)pLengths[dataType].tipVectorLength, f);       
      myBinFwrite(tr->partitionData[model].substRates, sizeof(double),  (size_t)pLengths[dataType].substRatesLength, f);
      ...
      if(tr->partitionData[model].protModels == LG4M || tr->partitionData[model].protModels == LG4X)
	{
	  ...
	  for(k = 0; k < 4; k++)
	    {
	      ...
	      myBinFwrite(tr->partitionData[model].frequencies_LG4[k], sizeof(double),  (size_t)pLengths[dataType].frequenciesLength, f);
	      myBinFwrite(tr->partitionData[model].tipVector_LG4[k], sizeof(double),  (size_t)pLengths[dataType].tipVectorLength, f);  
	      myBinFwrite(tr->partitionData[model].substRates_LG4[k], sizeof(double),  (size_t)pLengths[dataType].substRatesLength, f);    
	    }
	}
      ...
      //then pomo-related stuff      
      myBinFwrite(&(tr->partitionData[model].pomoFrequencies), sizeof(double), 4, f);
      myBinFwrite(&(tr->partitionData[model].pomoRates),       sizeof(double), 6, f);
      myBinFwrite(&(tr->partitionData[model].pomoPhi), sizeof(double), 1, f);
      myBinFwrite(&(tr->partitionData[model].pomoK), sizeof(double),   1, f);
      myBinFwrite(&(tr->partitionData[model].pomoN), sizeof(int),      1, f);      
    }
    
  if(ckp.state == MOD_OPT)
    {
      myBinFwrite(tr->likelihoods, sizeof(double), (size_t)tr->numberOfTrees, f);
...
static void readTree(tree *tr, FILE *f)
{
  ...
  myBinFread(tr->nodeBaseAddress, sizeof(node), (size_t)x, f);
   ...
   
  {
	...
	updateTipXVectors(tr, (size_t)model);
  }
...
static void readCheckpoint(tree *tr, analdef *adef)
{
  ...
  if(tr->constraintTree)
    myBinFread(tr->constraintVector, sizeof(int), 2 * (size_t)tr->mxtips, f);  
  ...
  myBinFread(tr->tree0, sizeof(char), (size_t)tr->treeStringLength, f);
  myBinFread(tr->tree1, sizeof(char), (size_t)tr->treeStringLength, f);
  ...
  if(tr->rateHetModel == CAT )
    {
	 ...
	 goes wrong */
      exa_off_t
	rPos = exa_ftell(f),      
	pPos  = rPos + (exa_off_t)(sizeof(int) * tr->originalCrunchedLength); 
      ...
      while(aIter != aEnd)
	{
	  if(aIter->procId == processID)
		...
		*partition = &(tr->partitionData[aIter->partitionId]); 
	      exa_off_t
		theOffset = pPos + (exa_off_t)((partition->lower + aIter->offset)  * sizeof(double)); 
	      ...
	      theOffset = rPos + (exa_off_t)((partition->lower + aIter->offset) * sizeof(int)); 
      ...
      exa_fseek(f, pPos + (exa_off_t)(tr->originalCrunchedLength * sizeof(double)) , SEEK_SET); 
    }
  ...
  for(model = 0; model < tr->NumberOfModels; model++)
	...
	dataType = tr->partitionData[model].dataType;
      ...
      myBinFread(tr->partitionData[model].perSiteRates, sizeof(double), (size_t)tr->maxCategories, f);
      myBinFread(tr->partitionData[model].EIGN, sizeof(double), (size_t)pLengths[dataType].eignLength, f);
      myBinFread(tr->partitionData[model].EV, sizeof(double),  (size_t)pLengths[dataType].evLength, f);
      myBinFread(tr->partitionData[model].EI, sizeof(double),  (size_t)pLengths[dataType].eiLength, f);  

      myBinFread(tr->partitionData[model].freqExponents, sizeof(double),  (size_t)pLengths[dataType].frequenciesLength, f);
      myBinFread(tr->partitionData[model].frequencies, sizeof(double),  (size_t)pLengths[dataType].frequenciesLength, f);
      myBinFread(tr->partitionData[model].tipVector, sizeof(double),  (size_t)pLengths[dataType].tipVectorLength, f);  
      myBinFread(tr->partitionData[model].substRates, sizeof(double),  (size_t)pLengths[dataType].substRatesLength, f);  
	  ...
	  for(k = 0; k < 4; k++)
	    {
	      ...
	      myBinFread(tr->partitionData[model].frequencies_LG4[k], sizeof(double),  (size_t)pLengths[dataType].frequenciesLength, f);
	      myBinFread(tr->partitionData[model].tipVector_LG4[k], sizeof(double),  (size_t)pLengths[dataType].tipVectorLength, f);  
	      myBinFread(tr->partitionData[model].substRates_LG4[k], sizeof(double),  (size_t)pLengths[dataType].substRatesLength, f);    
	    }
	}
	...
	makeGammaCats(tr->partitionData[model].alpha, tr->partitionData[model].gammaRates, 4, tr->useMedian); 
      ...
      myBinFread(&(tr->partitionData[model].pomoFrequencies), sizeof(double), 4, f);
      myBinFread(&(tr->partitionData[model].pomoRates),       sizeof(double), 6, f);
      myBinFread(&(tr->partitionData[model].pomoPhi), sizeof(double), 1, f);
      myBinFread(&(tr->partitionData[model].pomoK), sizeof(double),   1, f);
      myBinFread(&(tr->partitionData[model].pomoN), sizeof(int),      1, f);      
    }
    
  if(ckp.state == MOD_OPT)
    {
      myBinFread(tr->likelihoods, sizeof(double), (size_t)tr->numberOfTrees, f);
  ...
  while(impr)
      ...
      if(tr->searchConvergenceCriterion && processID == 0)
	{
	  ...
	  if(fastIterations % 2 == 0)	      
	    memcpy(tr->tree0, buffer, (size_t)tr->treeStringLength * sizeof(char));
	  else
	    memcpy(tr->tree1, buffer, (size_t)tr->treeStringLength * sizeof(char));	    
