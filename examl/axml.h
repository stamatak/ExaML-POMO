...
//mth caution for models with odd number of states we need 
//mth to replace these two below by the unaligned versions
...
//mth if the number of states is not a multiple of 4
//mth we unfortunately have to use the unaligned AVX load and store operations
//mth there might be a substantial performance degradation, albeit I have 
//mth not measured that yet!
...
#define PHI_MIN      0.0001
#define PHI_MAX      0.9999
...
#define TIP_TIP_CLV   3
#define TIP_INNER_CLV 4
...
#define GENERIC_32       6
#define GENERIC_64       7
//mth define a new data type called POMO
#define POMO_16          8
#define POMO_64          9
#define MAX_MODEL        10
...
typedef struct {
  ...
  double          **xVector;  
  double          **xTipVector; //POMO tip CLV 
  double          **xTipCLV;  //POMO tip CLV 
  double           *xResource;
  size_t           *xSpaceVector;  
...
typedef struct {
  ...
  double tr_lhCutoff;
  double tr_lhAVG;
  unsigned long tr_lhDEC;
  int    tr_NumberOfCategories;
  unsigned long   tr_itCount;  
  int    tr_doCutoff;
...
extern double evaluatePartialGeneric (tree *, size_t i, double ki, int _model);
...
extern boolean isPomo(int dataType);
...
extern void updateTipXVectors(tree *tr, size_t model);
...
extern void newviewGTRGAMMAPROT_AVX_LG4(int tipCase,
					...
					int *ex3, unsigned char *tipX1, unsigned char *tipX2, size_t n, 
