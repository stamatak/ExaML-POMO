...
void initTL(topolRELL_LIST *rl, tree *tr, int n)
{
  ...
  rl->max = n; 
  rl->t = (topolRELL **)malloc(sizeof(topolRELL *) * (size_t)n);

  for(i = 0; i < n; i++)
    {
      rl->t[i] = (topolRELL *)malloc(sizeof(topolRELL));
      rl->t[i]->connect = (connectRELL *)malloc((2 * (size_t)tr->mxtips - 3) * sizeof(connectRELL));
...
static topol  *setupTopol (int maxtips)
{
  ...
  if (! (tpl = (topol *) malloc(sizeof(topol))) || 
      ! (tpl->links = (connptr) malloc((2*(size_t)maxtips-3) * sizeof(connect))))
    {
  ...
  if (bt->ninit <= 0) 
    {
      ...
      bt->best = unlikely;
      bt->improved = FALSE;
      bt->byScore = (topol **) malloc(((size_t)newkeep+1) * sizeof(topol *));
      bt->byTopol = (topol **) malloc(((size_t)newkeep+1) * sizeof(topol *));
