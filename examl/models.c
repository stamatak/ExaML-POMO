...
void updateTipXVectors(tree *tr, size_t model)
{
  size_t    
    i,
    j;
  ...
  for(i = 1; i <= (size_t)tr->mxtips; i++)
	...
	*pv = tr->partitionData[model].xTipCLV[i];	  
      ...
      for(j = 0; j < width; j++)
	{
	  ...
	  for(l = 0; l < states; l++)
	    {
	      VECTOR_REGISTER _x = VECTOR_SET_ZERO();
...
static void updatePomoK(tree *tr, size_t model);
			...
			int model, int dataType, tree *tr)
{
  ...
  if(isPomo(dataType))
    {
      updatePomoK(tr, (size_t)model);
      updatePomoFreqs(tr, (size_t)model);
      updatePomoRates(tr, (size_t)model);
    }
...
{ 
  ...
  switch(tr->partitionData[model].dataType)
    { 
      //mth added POMO to Q matrix exponentiation
    case POMO_16:
    case POMO_64:
    case GENERIC_32:
    case GENERIC_64:
...
void makeGammaCats(double alpha, double *gammaRates, int K, boolean useMedian)
{
  ...
  double 
    ...
    alfa = alpha, 
    beta = alpha,
    *gammaProbs = (double *)malloc((size_t)K * sizeof(double));
...
void initRateMatrix(tree *tr)
{
  ...
  for(model = 0; model < tr->NumberOfModels; model++)
	...
	rates  = (states * states - states) / 2;
      ...
      switch(tr->partitionData[model].dataType)
	{
	  //mth added POMO to rate matrix value init
	case POMO_16:	  	  
	case POMO_64:
	  setRates(tr->partitionData[model].pomoRates, 6);
	  break;
	case BINARY_DATA:
	case DNA_DATA:
	case SECONDARY_DATA:
	  ...
	  switch(tr->multiStateModel)
		...
		for(j = 0; j < states; j++)
		  for(k = j + 1; k < states; k++)
		    tr->partitionData[model].substRates[it++] = (double)(k - j);			
		assert(it == rates);		
	      }
	      break;
	    case MK_MULTI_STATE:
...
static double pomoRateLookup(int i, int j, size_t model, tree *tr)
{
  //todo update later 
  //todo watch out for confusing rates with the relative mutation rates 
  //this function here will always return the relative rates!

  int 
    index,
    states = 4;

  assert(i < j && i < states && j < states);
   
  index = (i * states + j) - (((i + 1) * (i + 2)) / 2);

  assert(index >= 0 && index < 6);
   
  //printf("index: %d\n", index);

  return tr->partitionData[model].pomoRates[index];
}

//pomo K depends on the four plain, unscaled base frequencies 

static void updatePomoK(tree *tr, size_t model)
{
  double 
    *f = tr->partitionData[model].pomoFrequencies,
    result = 0.0;
  ...
  int 
    ...
    j, 
    k,
    n = tr->partitionData[model].pomoN; 

  for(i = 0; i < 4; i++)
    for(j = i + 1; j < 4; j++)      
      for(k = 1; k < n; k++)	//changed from j i to i j could be a problem confusion about scaled/unscaled rates 
	result += ((f[i] * f[j] * pomoRateLookup(i, j, model, tr)) / (double)(k)) + 
	  ((f[i] * f[j] * pomoRateLookup(i, j, model, tr)) / (double)(n - k));
  
  tr->partitionData[model].pomoK = result;
}

//pomo base freqs depends on the four plain unscaled base freqs, on phi and on K!

static void updatePomoFreqs(tree *tr, size_t model)
{
  double
    sum = 0.0,
    n = (double)tr->partitionData[model].pomoN,
    ratio =  tr->partitionData[model].pomoPhi / tr->partitionData[model].pomoK,
    *f = tr->partitionData[model].pomoFrequencies;
  ...
  for(l = 0; l < 4; l++)   
    {         
      tr->partitionData[model].frequencies[l] = f[l] * (1.0 - tr->partitionData[model].pomoPhi);  
      sum += tr->partitionData[model].frequencies[l];
    }

  // TODO state ordering correct like this?

  for(i = 0; i < 4; i++)
    for(j = i + 1; j < 4; j++)      
      for(k = 1; k < tr->partitionData[model].pomoN; k++)
	{
	  double 
	    v = ratio * 
	    ( (n * f[i] * f[j] * pomoRateLookup(i, j, model, tr)) / 
	      (double)(k * (tr->partitionData[model].pomoN - k))); 
	 
	  tr->partitionData[model].frequencies[l++] =  v;
	  
	  sum += v;
	}

 
  assert(l == tr->partitionData[model].states);

  
  assert(fabs(1.0 - sum) <= 0.0000001);      
}
...
static void updatePomoRates(tree *tr, size_t model)
{
  int 
    ...
    j,    
    l,
    states = (int)tr->partitionData[model].states, 
    stride = (int)tr->partitionData[model].pomoN - 1;
  ...
  //mth TODO fill in the real stuff!
  
  //mth first do mono -> poly transitions 
  for(i = 0, l = 0; i < 4; i++)           
    for(j = i+1; j < 4; j++, l++)
      {
	m[i][4 + l * stride] = 1.0;
	m[j][4 + ((l+1) * stride - 1)] = 1.0;
      }
  assert(l == 6);
  ...
  */
  ...
  for(i = 0, l = 0; i < states; i++)
    for(j = i + 1; j < states; j++)
      tr->partitionData[model].substRates[l++] = m[i][j];
}
...
static void initializeBaseFreqs(tree *tr)
{
  ...
  for(model = 0; model < (size_t)tr->NumberOfModels; model++)
    {     
      //mth if pomo only set/get the first 4 frequencies!
      size_t
	numFreqs = (isPomo(tr->partitionData[model].dataType))? 4 : (size_t)tr->partitionData[model].states;
      ...
      if(tr->partitionData[model].optimizeBaseFrequencies)
	{
	  ...
	  for(l = 0; l < numFreqs; l++)
		...
		tr->partitionData[model].pomoFrequencies[l] = f;
	...
	{	 
	  ...
	  memcpy(tr->partitionData[model].empiricalFrequencies, tr->partitionData[model].frequencies, sizeof(double) * numFreqs);
	  if(isPomo(tr->partitionData[model].dataType))
	    memcpy(tr->partitionData[model].pomoFrequencies, tr->partitionData[model].frequencies,  sizeof(double) * numFreqs);
	}
    }
  ...
  for(model = 0; model < (size_t)tr->NumberOfModels; model++)
	...
	dataType = tr->partitionData[model].dataType;
      ...
      if(isPomo(dataType))
	{
	  tr->partitionData[model].pomoPhi = 0.5; 

	  switch(dataType)
	    {
	    case POMO_16:
	      tr->partitionData[model].pomoN = 3;
	      break;
	    case POMO_64:
	      tr->partitionData[model].pomoN = 11;
	      break;
	    default:
	      assert(0);
	    }
	  
	  updatePomoK(tr, model);

	  updatePomoFreqs(tr, model);
	  
	  updatePomoRates(tr, model);
	}      
    }
...
{  
  ...
  //for POMO this call below overwrites the subst rates from initRateMatrix!
