...
static void getVects(tree *tr, unsigned char **tipX1, unsigned char **tipX2, double **x1_start, double **x2_start, int *tipCase, int model,
		     double **x1_gapColumn, double **x2_gapColumn, unsigned int **x1_gap, unsigned int **x2_gap, size_t offset, int *genericTipCase)
{
  int    
	  ...
	  if(isPomo(tr->partitionData[model].dataType))
	    {
	      ...
	      *x1_start = tr->partitionData[model].xTipVector[pNumber];
	      *x2_start = tr->partitionData[model].xTipVector[qNumber];
...
static void sumCAT_FLEX(int tipCase, double *sumtable, double *x1, double *x2, double *tipVector,
			unsigned char *tipX1, unsigned char *tipX2, size_t n, const int states)
{
  size_t
    i, 
    l;
...
static void sumGAMMA_BINARY(int tipCase, double *sumtable, double *x1_start, double *x2_start, double *tipVector,
                            unsigned char *tipX1, unsigned char *tipX2, size_t n);
static void coreGTRGAMMA_BINARY(const size_t upper, double *sumtable,
                                volatile double *d1,   volatile double *d2, double *EIGN, double *gammaRates, double lz, int *wrptr);
static void coreGTRCAT_BINARY(const size_t upper, size_t numberOfCategories, double *sum,
...
static void sumCAT_SAVE(int tipCase, double *sum, double *x1_start, double *x2_start, double *tipVector,
			unsigned char *tipX1, unsigned char *tipX2, size_t n, double *x1_gapColumn, double *x2_gapColumn, unsigned int *x1_gap, unsigned int *x2_gap);
...
static void sumGAMMA_NSTATE(double *sumtable, double *x1, double *x2, double *tipVector,
			    unsigned char *tipX1, unsigned char *tipX2, size_t n, const size_t numberOfStates, const size_t gammaRates, const int genericTipCase);
...
static void coreGTRGAMMAPROT_LG4(double *gammaRates, double *EIGN[4], double *sumtable, const size_t upper, int *wrptr,
				 volatile double *ext_dlnLdlz,  volatile double *ext_d2lnLdlz2, double lz, double *weights);

static void coreGTRGAMMA(const size_t upper, double *sumtable,
			 volatile double *ext_dlnLdlz,  volatile double *ext_d2lnLdlz2, double *EIGN, double *gammaRates, double lz, int *wgt);

static void coreGTRCAT(const size_t upper, size_t numberOfCategories, double *sum,
		       volatile double *d1, volatile double *d2, int *wgt,
		       double *rptr, double *EIGN, int *cptr, double lz);


static void coreGTRGAMMAPROT(double *gammaRates, double *EIGN, double *sumtable, const size_t upper, int *wgt,
			     volatile double *ext_dlnLdlz,  volatile double *ext_d2lnLdlz2, double lz);

static void coreGTRGAMMA_NSTATE(double *gammaRates, double *EIGN, double *sumtable, const size_t upper, int *wgt,
			 volatile double *ext_dlnLdlz,  volatile double *ext_d2lnLdlz2, double lz, const size_t numberOfStates, const size_t numberOfGammaRates);

static void coreGTRCATPROT(double *EIGN, double lz, size_t numberOfCategories, double *rptr, int *cptr, const size_t upper,
...
static void coreCAT_FLEX(const size_t upper, size_t numberOfCategories, double *sum,
			 volatile double *d1, volatile double *d2, int *wgt,
...
static void coreGAMMA_FLEX(const size_t upper, double *sumtable, volatile double *ext_dlnLdlz,  volatile double *ext_d2lnLdlz2, 
...
#pragma omp parallel
  ...
  for(m = 0; m < maxModel; m++)
      ...
      if(tr->td[0].executeModel[model] && width > 0)
	{
	  ...
	  getVects(tr, &tipX1, &tipX2, &x1_start, &x2_start, &tipCase, model, &x1_gapColumn, &x2_gapColumn, &x1_gap, &x2_gap, offset, &genericTipCase);
	  ...
	  switch(states)
	    ...
	    case 16:
	      ...
	      //mth do some precalculations for NR optimization and store them in an 
	      //mth array that can later be re-used

	      switch(tr->rateHetModel)
		{
		case GAMMA:
		  sumGAMMA_NSTATE(sumBuffer, x1_start, x2_start, tr->partitionData[model].tipVector,
				  tipX1, tipX2, width, 16, 4, genericTipCase);
		  break;
		case PLAIN:
		  sumGAMMA_NSTATE(sumBuffer, x1_start, x2_start, tr->partitionData[model].tipVector,
				  tipX1, tipX2, width, 16, 1, genericTipCase);
		  break;
		default:
		  assert(0);
		}
		...
		case GAMMA:
		  sumGAMMA_NSTATE(sumBuffer, x1_start, x2_start, tr->partitionData[model].tipVector,
				  tipX1, tipX2, width, 64, 4, genericTipCase);	      
		  break;
		case PLAIN:
		  sumGAMMA_NSTATE(sumBuffer, x1_start, x2_start, tr->partitionData[model].tipVector,
				  tipX1, tipX2, width, 64, 1, genericTipCase);	    
		  break;
		default:
		  assert(0);
		}
...
#pragma omp parallel
#endif
  {
	...
	{
	  size_t
	    rateHet = (size_t)discreteRateCategories(tr->rateHetModel),
	  ...
	  if(tr->rateHetModel == CAT)
		...
		assert(!tr->saveMemory);
		if(tr->rateHetModel == CAT)
		  coreGTRCAT_BINARY(width, (size_t)tr->partitionData[model].numberOfCategories, sumBuffer,
				    &dlnLdlz, &d2lnLdlz2,
		...
		if(tr->rateHetModel == CAT)
			       ...
			       assert(0 && "CAT model of rate heterogeneity is not implemented on Intel MIC");
  #else
		coreGTRCAT(width, (size_t)tr->partitionData[model].numberOfCategories, sumBuffer,
					    &dlnLdlz, &d2lnLdlz2, wgt,
		...
		if(tr->rateHetModel == CAT)
			       ...
			       assert(0 && "CAT model of rate heterogeneity is not implemented on Intel MIC");
  #else
		coreGTRCATPROT(tr->partitionData[model].EIGN, lz, (size_t)tr->partitionData[model].numberOfCategories,  tr->partitionData[model].perSiteRates,
					    rateCategory, width,
					    wgt,
					    &dlnLdlz, &d2lnLdlz2,
					    sumBuffer);
		...
		switch(tr->rateHetModel)
		  {
		  case GAMMA:
		    coreGTRGAMMA_NSTATE(tr->partitionData[model].gammaRates, tr->partitionData[model].EIGN,
					sumBuffer, width, wgt,
					&dlnLdlz, &d2lnLdlz2, lz, 16, 4);
		    break;
		  case PLAIN:
		    {
		      double 
			plain[1] = {1.0};
		      ...
		      coreGTRGAMMA_NSTATE(plain, tr->partitionData[model].EIGN,
					  sumBuffer, width, wgt,
					  &dlnLdlz, &d2lnLdlz2, lz, 16, 1);
		    }
		    break;
		  default:
		    assert(0);
...
static void topLevelMakenewz(tree *tr, double *z0, int _maxiter, double *result)
{
  ...
  do
      ...
      if(firstIteration)
	{
      ...
      {
	double 
	  *send = (double *)malloc(sizeof(double) * (size_t)tr->numBranches * 2),
	  *recv = (double *)malloc(sizeof(double) * (size_t)tr->numBranches * 2);		
  
	memcpy(&send[0],                dlnLdlz,   sizeof(double) * (size_t)tr->numBranches);
	memcpy(&send[tr->numBranches],  d2lnLdlz2, sizeof(double) * (size_t)tr->numBranches);
	...
	memcpy(dlnLdlz,   &recv[0],               sizeof(double) * (size_t)tr->numBranches);
	memcpy(d2lnLdlz2, &recv[tr->numBranches], sizeof(double) * (size_t)tr->numBranches);
...
/**** binary ***/
static void coreGTRCAT_BINARY(size_t upper, size_t numberOfCategories, double *sum,
                              volatile double *d1, volatile double *d2, 
                              double *rptr, double *EIGN, int *cptr, double lz, int *wgt)
{
  size_t i;
    ...
    { 
      ...
      for(j = 0; j < 4; j++)
        {                       
          ...
          __m128d tmpv = _mm_mul_pd(_mm_load_pd(pd0), _mm_load_pd(&sum[j * 2]));
          a0 = _mm_add_pd(a0, tmpv);
          a1 = _mm_add_pd(a1, _mm_mul_pd(tmpv, _mm_load_pd(pd1)));
          a2 = _mm_add_pd(a2, _mm_mul_pd(tmpv, _mm_load_pd(pd2)));                          
        }
